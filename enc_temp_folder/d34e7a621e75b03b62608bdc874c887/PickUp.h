...
class ACocinaSimulatorCharacter;
...
public:
	...
	// FIX: bIsHeld y HoldingCharacter comparten el mismo RepNotify para
	//      que cuando llega bIsHeld=true ya tengamos HoldingCharacter valido
	...
	// FIX: personaje que lleva el item — replicado para que los clientes
	//      puedan hacer attach visual en OnRep_IsHeld
	UPROPERTY(ReplicatedUsing = OnRep_IsHeld, BlueprintReadOnly, Category = "Pickup")
	ACocinaSimulatorCharacter* HoldingCharacter = nullptr;

	// Referencia al spawner que creo este item (solo servidor, no replicado)
	UPROPERTY()
	AIngredientSpawner* OwnerSpawner = nullptr;

	// ---- Funciones publicas -----------------------------------------------

	void PickUpItem(USceneComponent* HoldPoint, ACocinaSimulatorCharacter* Character);
